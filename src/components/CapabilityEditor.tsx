...
export function CapabilityEditor({ projectId, sandboxId, capabilities }: { projectId: string; sandboxId: string; capabilities: Capability[] }) {
  ...
  return <div className="policy-workbench">
    ...
    <section className="policy-panel"><h2>Capability policy</h2><p>Define how external agents may use this operation.</p><div className="policy-options">{(Object.keys(labels) as Policy[]).map((policy) => <button key={policy} className={selected.policy === policy ? `policy-option active ${policy}` : `policy-option ${policy}`} onClick={() => updateSelected({ policy })}><span>{policy === "read_only" ? <IconCheck /> : policy === "approval_required" ? <IconCircleKey /> : policy === "prohibited" ? <IconAlertTriangle /> : <IconShieldCheck />}</span><strong>{labels[policy]}</strong><small>{policy === "read_only" ? "No state changes" : policy === "reversible" ? "Writes with a recovery path" : policy === "approval_required" ? "Principal approval before use" : "Unavailable to agents"}</small></button>)}</div><label className="enable-toggle"><input type="checkbox" checked={selected.enabled} onChange={(event) => updateSelected({ enabled: event.target.checked })} /> Include in sandbox</label></section>
