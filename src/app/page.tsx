...
export default async function HomePage() {
  ...
  return (
    <main className="landing">
      <nav className="landing-nav"><Link href="/" className="wordmark">Agent Access</Link><div><Link href="/docs">Docs</Link>{userId ? <Link className="button primary" href="/dashboard">Open console</Link> : <><Link className="button ghost" href="/sign-in">Sign in</Link><Link className="button primary" href="/sign-up">Start building</Link></>}</div></nav>
      <section className="hero">
        ...
        <div className="hero-actions">{userId ? <Link className="button primary large" href="/dashboard/import">Import OpenAPI <IconArrowRight size={18} /></Link> : <Link className="button primary large" href="/sign-up">Publish an agent sandbox <IconArrowRight size={18} /></Link>}<Link className="button ghost large" href="/docs">Read the machine docs</Link></div>
